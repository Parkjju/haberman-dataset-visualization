...
$$
\hat{S}(t) = \prod_{i\leq t}(1-\frac{d_{i}}{n_{i}}) = \prod_{i\leq t}p_{t} = p_{1} \times p_{2} \times \cdot\cdot\cdot \times p_{t-1} \times p_{t} = \hat{S}(t-1) \times p_{t}
$$
...
1. `SELECT RANDOM DATA` 버튼을 클릭하여 `haberman's survival dataset` 306개의 샘플 중 하나를 선택하여 CoxPH 모델에 피팅된 데이터를 시각화할 수 있다.
2. `데이터 추가하기` 체크박스를 클릭하면 새로운 샘플을 추가할 수 있다. 나이, 수술이 이루어진 연도의 뒤 두자리, 양성 림프종 발견 개수와 최종적으로 해당 환자의 생존 여부를 입력한다.
    - 데이터 입력 후 `APPEND DATA` 버튼을 클릭하면 기존 데이터셋에 입력 데이터 행을 추가하여 CoxPH 모델 피팅을 다시 처리한다.
    - 이후 데이터 row를 하나 랜덤으로 선택하여 피팅된 CoxPH 모델을 기준으로 생존확률을 새롭게 예측한다.
